// display_bsp.h —— ST7305 全反射屏驱动（移植自微雪官方例程，未改动逻辑）
#pragma once
...
private:
    ...
    bool RLCD_WaitTE(uint32_t timeout_ms);   // 等面板 TE 帧起始脉冲（忙等，超时返回 false）

public:
    DisplayPort(int mosi, int scl, int dc, int cs, int rst, int width, int height,
                spi_host_device_t spihost = SPI3_HOST, int te_pin = -1);
    ~DisplayPort();
    void RLCD_Init();
