// i2c_equipment.cpp —— SHTC3 温湿度 + PCF85063 RTC
// SHTC3 部分移植自微雪官方例程（未改动）；
// RTC 部分官方使用 SensorLib 的 SensorPCF85063，本工程为减小体积改为
// 使用自写的最小驱动 rtc_pcf85063（接口行为保持一致）。
#include <stdio.h>
#include <esp_log.h>
...
// ===================== PCF85063 RTC =====================
// 使用自写的最小驱动替代官方的 SensorLib::SensorPCF85063
static i2c_master_dev_handle_t I2cRTCdev = NULL;
static I2cMasterBus           *I2cbus_   = NULL;
static Pcf85063Drv             rtc;

// Pcf85063Drv 的 IO 适配层：把「寄存器 + 缓冲」读写翻译到 I2cMasterBus
static bool RtcIOCallback(void *user, uint8_t reg, uint8_t *buf, size_t len, bool is_write) {
    (void)user;
    int ret;
    if (is_write) {
        ret = I2cbus_->i2c_write_buff(I2cRTCdev, reg, buf, (uint8_t)len);
    } else {
        ret = I2cbus_->i2c_read_buff(I2cRTCdev, reg, buf, (uint8_t)len);
    }
...
void Rtc_Setup(I2cMasterBus *i2cbus,uint8_t dev_addr) {
    ...
    if (rtc.begin(RtcIOCallback, NULL)) {
        ESP_LOGI("rtc", "PCF85063 init OK");
    } else {
        ESP_LOGE("rtc", "PCF85063 init FAILURE");
    }
}
...
void Rtc_GetTime(rtcTimeStruct_t *time) {
    int y,mo,d,h,mi,s,w;
    if (rtc.getDateTime(&y,&mo,&d,&h,&mi,&s,&w)) {
        time->year = y; time->month = mo; time->day = d;
        time->hour = h; time->minute = mi; time->second = s;
        time->week = w;
    }
}
